...
const ArchiveClient = ({ open, setOpen, id }) => {
  ...
  const handleConfirm = () => {
    setSubmitting(true);
    api
      .put(`${httpsEndpoint}/client/${id}/archive`, {
        headers: {
      ...
      .catch(({ response }) => {
        ...
        if (
          error ===
          "You can't archive this client. Shifts are associated with it."
        ) {
          setHandleOpenSnackbar({
            open: true,
            severity: "error",
            message:
              "Vous ne pouvez pas archiver ce client. Des plages horaires lui sont associées.",
          });
        } else {
          setHandleOpenSnackbar({
            open: true,
    ...
    <>
      ...
      <BSModal open={open} onClose={() => setOpen(false)}>
        <Typography variant="h6" component="h2">
          Êtes-vous sûr(e) de vouloir archive ce client ?
        </Typography>
